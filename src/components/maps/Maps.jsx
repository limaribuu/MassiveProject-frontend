...
const Maps = ({
  ...
  const locations = [
    ...
    {
      lat: -2.9916400715717937,
      lng: 104.7634990568582,
      name: "Jembatan Ampera",
      slug: "ampera",
      address:
        "9/10 Ulu, Kecamatan Seberang Ulu I, Kota Palembang, Sumatera Selatan",
      img: "/reco/ampera.png",
      description: "Jembatan Ampera yang menjadi ikon utama kota Palembang",
    },
    {
      lat: -2.989138846829649,
      lng: 104.76040568730309,
      name: "Monpera",
      slug: "monpera",
      address:
        "Jl. Merdeka No.1, 19 Ilir, Kec. Bukit Kecil, Kota Palembang, Sumatera Selatan 30113",
      img: "/reco/monpera.png",
      description: "Monumen perjuangan rakyat Palembang melawan penjajah",
    },
  ].map((l) => ({ ...l, slug: l.slug || toSlug(l.name) }));
  ...
  useEffect(() => {
    ...
    locations.forEach((loc) => {
      const slug = loc.slug || toSlug(loc.name);
      const btnId = `detail-btn-${slug}`;
      ...
      // simpan marker berdasarkan nama & slug
    ...
    setMap(mymap);
  }, [leafletLoaded]); // sama seperti sebelumnya
  ...
  // 🔹 FOKUS OTOMATIS KE DESTINASI BERDASARKAN SLUG (TANPA POPUP)
  useEffect(() => {
    if (!map || !focusSlug) return;
    const target = locations.find((loc) => loc.slug === focusSlug);
    if (!target) return;

    // hanya center & zoom, tidak openPopup
    map.setView([target.lat, target.lng], 17);
  }, [map, focusSlug, locations]);
    ...
    >
      ...
      {showBack && (
        <button
          ...
          className={cx(
            isPage
              ? "fixed top-3 left-12.5 z-1100"
              : "absolute top-3 left-5 z-20",
